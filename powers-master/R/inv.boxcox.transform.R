#' Inverse Box-Cox Transform Function
#'
#' Given a variable, the inverse Box-Cox power transformation is obtained.
#'
#' @param x vector that is to be transformed under the inverse box-cox function. All x must be positive.
...
#'
#' @return A vector of the values that have been transformed. If lambda is a 0 value, returns the `exp(x)`. If lambda is non-0 value, returns `(lambda*x + 1)^(1/lambda)`.
#' @export

inv.boxcox.transform <- function(x, lambda = 0, makepos=0) {
    # Common error checks
    ...
    # Inverse Box-Cox Function Computations
    if(length(lambda) != 1) return(inv.boxcox.tranform.mat(x, lambda))
    if(lambda != 0) return((lambda*x + 1)^(1/lambda))
    else return(exp(x))
}
